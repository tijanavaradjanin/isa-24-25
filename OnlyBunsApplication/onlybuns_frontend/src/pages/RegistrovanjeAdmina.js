...
export default function RegistrovanjeAdmina() {
  ...
  const checkEmailExists = async () => {
    try {
      const response = await fetch("http://localhost:8080/adminkompanija/emails");
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        const response = await fetch('http://localhost:8080/kompanija');
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const data = await response.json();
        // Update state with fetched company data
        setCompanies(data);
      } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    const kompanija = companies.find(company => company.naziv === selectedCompany);

    const korisnik = {ime, prezime, grad, drzava, broj, email, password, info, kompanija}
    console.log(korisnik);

    setErrorMessage('');


    fetch("http://localhost:8080/adminkompanija/save",{
      method:"POST",
        ...
        >
          ...
          <Box component="form" onSubmit={handleSubmit} noValidate sx={{ mt: 1 }}>
            ...
            <TextField
              ...
              <option key={company.id} value={company.naziv}>
                {company.naziv}
              </option>
              ))}
            </TextField>
