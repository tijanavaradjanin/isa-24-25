...
export default function KreiranjeObjave() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (grad && drzava && ulica && broj) {
      ...
      setError("Morate uneti celu adresu ili odabrati lokaciju na mapi.");
      setLoading(false);
      return;
    }
      ...
      .then((response) =>
        response.ok
          ...
          : response.json().then((data) => {
              throw new Error(data.message || 'Greška pri kreiranju objave.');
            })
      )
      .then((data) => {
        navigate('/mojeObjave');
    ...
    <div style={{ position: "relative" }}>
      ...
      <Box sx={{
        ...
        <form className="kreiranje-objave-form" onSubmit={handleSubmit}>
          ...
          {error && (
            <Box sx={{ color: 'red', mt: 2, fontWeight: 'bold' }}>
              {error}
            </Box>
          )}
        </form>
